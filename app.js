...
WMN3 DNS PRO
FULL APP.JS FINAL
LOGIN + CREATE USER FIX + ADMIN + HISTORY + LIVE NOTIF
Replace old app.js بالكامل بهذا الملف
===================================================== */
...
function loadUsers(){
  USERS = JSON.parse(localStorage.getItem("usersDB")) || USERS;
}
...
function login(){
  ...
  if(!found){
    if(byId("errorMsg")) byId("errorMsg").innerText = "Invalid Login";
    return;
  }

  if(found.blocked){
    if(byId("errorMsg")) byId("errorMsg").innerText = "Blocked User";
    return;
  }
...
function openDashboard(user){
  ...
  if(byId("loginPage")) byId("loginPage").style.display = "none";
  if(byId("dashboard")) byId("dashboard").style.display = "block";
...
function renderHistory(){
  ...
  h.forEach(item=>{
    ...
    div.onclick = ()=>{
      if(byId("output")) byId("output").value = item.data;
    };
